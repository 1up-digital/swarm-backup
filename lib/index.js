...
class SwarmBackup {
  constructor(gateway, privateKey, batchID, timeout) {
    this.gateway = gateway;
    this.privateKey = privateKey;
    this.batchID = batchID;
    this.timeout = timeout;
    this.feed = new Feed(privateKey, this.batchID, this.gateway, this.timeout);
  }
  ...
  async restore(dataKey) {
    try {
      if (typeof dataKey === "undefined")
        throw new Error("dataKey must be defined");
