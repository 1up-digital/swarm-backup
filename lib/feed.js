...
class Feed {
  constructor(privateKey, batchID, gateway, timeout) {
    this.privateKey = privateKey;
    this.bee = new BeeJs.Bee(gateway, { timeout: timeout });
  ...
  async set(topicName, data) {
    ...
    let dr = await this.bee.uploadData(this.batchID, p);
    let r = await w.upload(this.batchID, dr.reference);
    return dr;
  }
